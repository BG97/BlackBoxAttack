...
for i in range(15):
  ...
  evaluated_gradients = gradient(model_B,dataset_D,labels)
  #print("eval ",len(evaluated_gradients[0]),"dataset's shape",dataset_D.shape)
  for j in range(len(dataset_D)):
    #print(np.sign(evaluated_gradients[0][j]))
    #print(dataset_D[j])
    if np.random.randint(2)==0:
      temp[j]=dataset_D[j]+128*np.sign(evaluated_gradients[0][j])
    else:
      temp[j]=dataset_D[j]-128*np.sign(evaluated_gradients[0][j])
  labels=np.concatenate((labels,labels),0)
  dataset_D=np.concatenate((dataset_D,temp),0)
  model_B.fit(dataset_D,labels,batch_size=32,epochs=10)
  final_gradient=gradient(model_B,dataset_D,labels)
  final_dataset=dataset_D+epsilons*np.sign(final_gradient)
  #final_predict=model.predict(final_dataset)
  
  print('________________accuracy______________',model.evaluate(final_dataset[0,:,:,:,:],labels))



#print(dataset_D.shape)
#model_B.fit(dataset_D,labels,batch_size=32,epochs=5)

#final_gradient=gradient(model_B,restOfset,restOflabels)
#final_dataset=restOfset+25*np.sign(final_gradient)
#final_predict=model.predict(final_dataset)
#print('________________accuracy______________',model.evaluate(final_dataset[0,:,:,:,:],restOflabels))


model_B.save(sys.argv[3])
